...
1. 历史上 load 只算等 CPU 的可运行任务，但这无法反映「系统因等磁盘而整体卡住」的情况。1993 年起 Linux 把 `D`（TASK_UNINTERRUPTIBLE，多为等磁盘 I/O 或被持有的内核锁）也计入，使 load 更像「系统综合繁忙度」而非纯 CPU 指标。典型「load 高但 CPU 闲」场景：一块慢盘或挂死的 NFS 挂载点，几十个进程卡在 D 状态等 I/O，CPU `%idle` 很高但 load 飙到几十。列出 D 进程及阻塞函数：`ps -eo pid,stat,wchan:30,comm | awk '$2 ~ /D/'`（wchan 列即阻塞的内核函数，如 `io_schedule`）。

2. USE 对每种资源问：Utilization（利用率，有多少时间在忙）、Saturation（饱和度，有多少额外工作在排队）、Errors（错误，有无错误计数增长）。饱和度更能说明问题，是因为利用率 100% 不一定是问题（可能就该满载），但有大量工作在排队、延迟飙升才是真正「忙不过来」。CPU 三例：U=`mpstat` 的 `%idle`（反向）；S=`vmstat` 的 `r` 列 / `/proc/pressure/cpu`；E=`perf stat` 的 machine check / dmesg 的 MCE。

3. IPC=0.25 极低说明 CPU 虽被打满，但大量周期在停顿等待（cache miss 风暴、访存延迟，或自旋锁空转），并不是在高效执行指令——「忙但没干活」。优化方向是数据局部性（减少 cache miss，用 `perf c2c` 查伪共享、调整数据结构布局）或消除锁竞争，而非加算力。加 CPU 核数没用，因为瓶颈是单核内的访存/停顿效率，更多核只会复制同样的低效。

4. X 轴不是时间轴，而是采样栈按字母序排列后各栈占总采样的比例（越宽=占 CPU 越多）；Y 轴是调用栈深度（顶部是正在执行的函数）。用 `-F 99` 而非 100 是为避免与系统里恰好 100Hz/50Hz 的周期性任务「锁相/共振」，用质数频率减少采样偏差。

5. `strace` 基于 ptrace，每个 syscall 都陷入跟踪器，开销极大（数倍到数十倍），适合低负载或离线调试、用 `-c` 看统计；`perf trace` 基于 perf_events/tracepoint，开销低一个量级，是生产跟踪 syscall 的首选；bpftrace（eBPF）开销最低、可任意 probe 并在内核内聚合，适合极高频生产场景。不能在生产高 QPS 进程上随手 `strace -p`，因为 ptrace 的逐 syscall 陷入会让目标慢几倍甚至触发雪崩。

6. 不能。NVMe/blk-mq 多队列设备能并行处理多个请求，`%util=100%` 只表示「任意时刻都有 I/O 在飞」，不等于队列满或饱和。真正的饱和度看 `aqu-sz`（平均队列深度）和 `await`（平均完成时间）。与机械盘不同的原因：传统单队列机械盘一次只能处理一个请求，`%util` 接近 100% 即饱和；多队列 SSD 可并发，`%util` 失去了饱和判据的意义。

7. 现象组合（CPU 大量 idle、`%wa` 偏高、`b` 队列稳定在 70、无 swap）强烈指向 I/O 象限。诊断序列：
   - `vmstat 1 5` —— 复核 `b`（不可中断阻塞数）、`wa`、`si/so`，确认是 I/O 等待而非 CPU/内存。
   - `ps -eo pid,stat,wchan:30,comm | awk '$2 ~ /D/'` —— 列出 D 状态进程，看 `wchan` 是否都指向 `io_schedule`/块层/NFS/FUSE 函数，定位是本地盘还是远端存储。
   - `iostat -xz 1 5` —— 看哪块盘 `await` 高、`aqu-sz`（队列深度）大，确认设备饱和（NVMe 别只看 `%util`）。
   - `pidstat -d 1 5` —— 找出哪个进程在狂读写（`kB_rd/s`/`kB_wr/s`）。
   - `cat /proc/pressure/io` —— 用 PSI io 量化失速占比佐证。
   - 进一步用 `biolatency`/`biosnoop`（eBPF）看 I/O 延迟分布与随机/顺序、是否 fsync 风暴。
   大概率定位到 I/O 象限（慢盘、fsync 风暴或卡死的远端存储），扩 CPU 无效。

8. 平均资源都不高的周期性毛刺，常见怀疑方向与验证：
   - 调度延迟：CPU 超卖或 cgroup `cpu.max` 周期性被打满，任务可运行却上不了 CPU。用 `runqlat`（bcc）看调度延迟分布直方图——若有一条尾巴拖到 ~100ms，正是毛刺来源；用 `perf sched latency` 看每任务最大调度延迟（Maximum delay）和逐事件细节。还可看 `cpu.stat` 的 `nr_throttled`/`throttled_usec`（L17）。
   - 网络重传：偶发 TCP 重传/RTO 导致单请求陡增。用 `ss -ti` 看具体连接的 rtt、cwnd、retrans，用 `tcpretrans`（eBPF）逐个抓重传事件，对照毛刺时刻。
   - GC / 缓存回收：运行时 GC 暂停（语言相关，Go 用 pprof/trace）或内核 page cache 周期性回收（看 `/proc/vmstat` 的 `pgscan`/`pgsteal`、PSI memory）造成 stop-the-world 或访存变慢。
   做法是把毛刺时刻与上述工具的时间序列对齐，哪个指标在毛刺时同步抬头即为元凶。
