...
> 难度：⭐⭐⭐⭐⭐
> 预计阅读时间：80 分钟
> 内容基准：2026 年 6 月（Kubernetes 1.32/1.33、PSA GA、Kyverno 1.13+、OPA Gatekeeper 3.18、Cosign 2.x、Falco 0.39+、Tetragon 1.x、SLSA v1.1）
...
`view` 默认**不能看 Secret**——内置 `view` 角色出于提权防护，从设计上就不授予 Secret 读权限（与某个特定版本的修正无关）。要看 Secret 必须 `edit` 或自定义 Role。
