...
```js
{ $sort: {...} }   // 自 6.0 起 allowDiskUseByDefault 默认 true，超 100MB 自动落盘
```
...
每个 stage 中间结果默认上限 100MB（含 $sort、$group 等内存阶段）。自 MongoDB 6.0 起，由服务端参数 `allowDiskUseByDefault` 控制（默认 true）——默认情况下超 100MB 的阶段会自动落盘到临时文件，而非报错：

- 可在单条查询用 `{ allowDiskUse: false }` 关闭
- 仅当 `allowDiskUseByDefault=false` 时才默认报 `QueryExceededMemoryLimit`，此时需加 `allowDiskUse: true` 用磁盘做
...
5. $facet 是并行还是串行？
6. $out 与 $merge 区别？
7. 聚合 100MB 内存限制：allowDiskUseByDefault 默认值是什么？怎么按需关闭落盘？
