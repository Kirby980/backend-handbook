...
1. 全 broker 升级到 4.3 二进制（仍宣称 4.2 协议）
2. 一个个 broker 改 inter.broker.protocol.version=4.3 重启
3. 完成
...
flowchart TD
    Q[需求]
    Q -->|生态成熟 + 标准| K[Apache Kafka 4.3.x]
...
- **KIP-848 已 GA（4.0）**：Consumer rebalance 新协议落地，不再 stop-the-world；后续推进全面普及
- **Queue 语义（KIP-932 share groups）已 production-ready（4.2）**：原生支持队列模式（per-message ack）；在途增强如 DLQ for queues（KIP-1191，targeting 4.4）
- **KIP-1071 Streams Rebalance Protocol**：Streams 侧新 rebalance 协议持续推进
...
```mermaid
graph TD
    Kafka[Apache Kafka 4.3.x<br>主流默认]
    Kafka --> Ops[运维]
