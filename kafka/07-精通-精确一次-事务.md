...
2. 无 `transactional.id` 的幂等 producer 由任一 broker 分配一个唯一的 **PID**；只有事务型 producer 才由 transaction coordinator 分配 PID 与 epoch
...
- 等 transaction.timeout.ms 触发 abort（producer 默认 60s；上限受 broker transaction.max.timeout.ms 默认 15min 约束）
