...
ZK→KRaft 迁移工具自 **Kafka 3.6** 引入、**3.7** 起官方生产可用（3.4 已引入 bridge 概念但当时未正式可用），**3.9 是推荐的 final bridge release**（迁移工具最完善）。直接从 2.x 迁不行，要先升级到 3.x。
...
3.4 引入概念、3.6 引入迁移工具、3.7 生产可用。让一个集群同时有：
