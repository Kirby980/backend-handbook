...
看起来很自然——但后端真正发生的事是：**每一轮都把整段历史重新发回模型**。30 轮对话，假如每轮 800 tokens，第 30 轮的请求里 input 已经是 24,000 tokens。如果用 Opus 4.8（$15/M input），单这一次 input 就要 $0.36；如果是 100 轮的客服对话，单个会话 input 成本可能突破 $5 美金。
...
2026 年 5 月，Claude Sonnet 4.6 已经支持 1M context window，Gemini 3 Pro 支持 1M（2M 为上一代 Gemini 1.5 Pro 的能力）。听起来"再也不用管 context 了"——但工程现实正好相反：**context window 越大，浪费就越大**。本章拆开生产级长对话管理的全套技术。
...
| 轮数 | 累计 input tokens | Opus 4.8 input 成本 | Sonnet 4.6 input 成本 |
|---|---|---|---|
| 1 | 2,000 | $0.030 | $0.006 |
...
| Input tokens | Sonnet 4.6 TTFT | Opus 4.8 TTFT |
|---|---|---|
| 2k | ~300ms | ~600ms |
| 16k | ~800ms | ~1.5s |
| 64k | ~2s | ~4s |
...
```
模型: Sonnet 4.6（默认）/ Opus 4.8（高质量需求）
