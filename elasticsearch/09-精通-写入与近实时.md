...
1. GET by _id 是强一致：它会查 translog，写入立即落 translog，即使文档还在内存 buffer、未 refresh 也能取到。SEARCH 是近实时：必须等下一次 refresh（默认 1s）把内存 buffer 转成可搜 segment 后才能命中。

2. translog 是 WAL：写文档先 append translog 再更新内存 buffer，崩溃后从最近 commit 点重放 translog 重建未落盘 segment 的状态，保证已 ack 的写不丢。`durability=async` 改为按 sync_interval（默认 5s）批量 fsync，提升写吞吐 30-50%，代价是掉电时最多丢最近一个 sync 间隔内已 ack 的写。

3. 1MB：每批管理开销相对大、吞吐起不来。15MB：甜点区，一次网络往返摊薄开销、coordinator 内部按 shard 并发分发。100MB：单批占内存大、GC 压力大、可能触发 indexing pressure 反压甚至 OOM、单批失败重试成本高。经验 5-15MB。

4. `refresh=true` 每次写都立即生成一个很小的新 segment，导致 segment 数爆炸、合并跟不上、查询和写入都变慢。正确做法是 `refresh=wait_for`（等下次自动 refresh）或批量写后统一 _refresh。

5. 写入吞吐下降（大致从 X 降到约 X×0.7）。因为写入是同步等所有 in-sync 副本 ack，副本=2 时 primary 要 fanout 给 2 个副本并等最慢者，网络/IO 放大和等待开销更大；但读吞吐相应提升。

6. 应用层应捕获 429（circuit_breaking_exception）并做**指数退避重试**（不要立刻猛重试），同时降低 bulk 大小/并发；持续 429 说明集群供不应求，要加节点或降客户端并发。429 是临时拒绝、必须重试；400（格式错误）不能重试。

7. 临时降级 5 项：`refresh_interval: -1`、`number_of_replicas: 0`、`translog.durability: async`、`translog.flush_threshold_size` 调大（如 1-2gb）、`merge.scheduler.max_thread_count` 适当调大（SSD）。导入完成后恢复并 force_merge。

8. Lucene 没有原地更新——update 等于"读出原 doc（从 _source）+ merge 新字段 + 删除旧 doc + 写入新 doc（新 _version）"。比 insert 多了"读旧 + 删旧"两步，所以慢 30-100%，且旧版本要等合并才物理清理。

9. delete_by_query 是 scroll + 逐条删除（每条只打 tombstone，要等合并才释放空间），数据量大时极慢且制造大量 deleted_doc。时序数据用 rollover 把每段时间分到独立索引，过期时直接 delete 整个索引（接近 O(1)，立即释放空间），远比逐条删除高效。

10. 设计：
- **节点**：master-only 3 个；data_hot 多节点（NVMe SSD、大内存，heap 30GB、其余留 page cache），按 50 亿×2KB≈10TB/天、单 shard 30-50GB 估算需 ~200-300 主 shard/天（含向量更大，可适当增 shard）；向量字段用 int8_hnsw 量化降内存；可选 coordinating-only 节点承担查询归并。
- **写入策略**：bulk 5-15MB、多 client 并发（≈data 节点数×2）、`refresh_interval: 30s`、`translog.durability: async`、replica 导入期可降；向量写入用 m=16/ef_construction=100 平衡建图速度；按天 rollover + ILM（hot→warm force_merge→cold/frozen→delete）。
- **监控告警**：indexing pressure rejections、write 线程池 queue/rejected、heap.percent>80%、Old GC 频率、disk watermark、merge 时间占比、segment 数、translog 大小、replica lag、circuit breaker tripped。
