...
1. 给 30GB（不超过 32GB）。不是 50%（32GB）因为：堆 ≤ 32GB 时 JVM 用 Compressed OOPs（压缩对象指针）省一半指针内存，超过会切回 64-bit 指针、对象头变大、实际可用堆反而下降；另一半物理内存要留给 OS page cache（Lucene segment 靠 mmap + page cache 读），不是给 heap。

2. 600 不是硬上限而是经验阈值（默认 `cluster.max_shards_per_node` 是 1000）。来源：经验法则"每 GB heap 容纳 < 20 个 shard"，30GB heap × 20 ≈ 600。每个 shard 是独立 IndexWriter/Reader，占文件句柄、cluster state 元数据、内存，过多会拖慢集群。

3. indexing pressure 跟踪"正在处理的写请求占用的内存字节"，超过 `indexing_pressure.memory.limit`（默认 10% heap）就拒绝新请求；write 线程池 rejected 是线程池队列满后的拒绝。两者都表现为写入被拒（429），indexing pressure 是按内存维度的反压，write 队列是按并发任务维度，常同时出现，根因都是写入供大于求。

4. text 字段默认无 doc_values，对它聚合/排序需要把字段加载为 fielddata 到 JVM heap（按词构造列式结构），大字段会吃光堆且很慢。改 mapping：加一个 keyword 子字段（`fields.keyword`，带 ignore_above），聚合走 `name.keyword`（用 doc_values，不占 heap）。

5. build_scorer 耗时高常见原因：query rewrite/构造 scorer 代价大——如 wildcard/regexp/prefix 前缀通配展开成海量 term、大 terms 查询、范围查询命中海量词项，或没有用上 filter cache 反复构造。优化：避免前导通配、用 keyword/wildcard 类型、把不评分条件放 filter 享受缓存。

6. 至少 3 个根因 + 验证：(1) mapping 字段爆炸/cluster state 大——`GET _cluster/stats` 看字段数、cluster_state_size；(2) shard 过多——`_cat/allocation` 看单节点 shard 数对比 heap；(3) 大聚合/fielddata——slow log + profile 看是否 terms size 巨大或 text fielddata 加载，`_nodes/stats/breaker` 看 fielddata breaker；另可看大 bulk + 多副本占 indexing buffer。

7. shrink 减少 shard 数：warm 阶段不再写入，shard 少则查询 fan-out 小、元数据/句柄少，查询更快。forcemerge max_num_segments=1 把 segment 合成一个：物理清理 deleted_doc、减少查询时扫描的 segment 数、压缩磁盘。两者只在停止写入的 warm 索引上做才安全。

8. parent circuit breaker 触发返回 HTTP 429（circuit_breaking_exception，有时表现为 503）。客户端应该重试——但要带退避，并最好降低请求规模（减小 bulk/聚合）；它是临时保护（防 OOM），盲目无退避猛重试只会持续触发。

9. 两个典型场景：(1) 大聚合/复杂查询——归并阶段在 coordinating 节点完成，耗 heap+CPU，隔离避免影响 data 节点；(2) 大量客户端/高 QPS 入口或 Kibana 后端——用专用 coordinating 做负载均衡入口，避免客户端连接打满 data 节点。

10. 单集群跨 region 是反模式：master 选举的 quorum、节点心跳、shard 复制都对延迟敏感，跨 region RTT（50-100ms+）会导致心跳超时、选举抖动、复制等待拖慢写入、cluster state 推送变慢，集群极不稳定。正确做法是每个 region 一个独立集群，用 CCR（跨集群复制）做数据同步、CCS（跨集群搜索）做联邦查询。
