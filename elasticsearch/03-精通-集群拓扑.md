...
1. master 选举要多数派（quorum = floor(N/2)+1），奇数个能在相同容错下减少节点浪费、避免选票平分。3 个 master 挂 1 个：剩 2 个仍达到 quorum(2)，正常工作并能选主。挂 2 个：剩 1 个不足 quorum，集群停止接受元数据变更（已分配数据的查询仍能继续，但无法创建索引、分配 shard 等）。

2. coordinating-only 节点专门接收请求、做 fanout 与归并（merge/排序/聚合），把"归并的 CPU/内存/GC 压力"从 data 节点剥离，让 data 节点专注查询自己的 shard。小集群（data 节点少、查询简单）没必要拆——多一跳网络反而增加简单请求的延迟，收益不明显。

3. hot：CPU 高、内存高、NVMe/SSD、IO 极高（承担写入+refresh+merge+热查询）；warm：CPU 可降、内存中、SSD，只读无写入；cold：IO 低、HDD 即可，磁盘成本是主要考量，查询慢可接受；frozen：本地磁盘只放缓存，主体在对象存储，磁盘需求极小但冷启动查询慢（秒到几十秒）。

4. 50GB 是单 shard 的经验甜区，兼顾 recovery/rebalance 速度、合并代价和查询并行度。设 100GB 会让单 shard 过大：节点故障后 recovery、shard 迁移都明显变慢，force_merge 等操作耗时更长，且单 shard 内存/句柄占用更高，影响集群弹性。

5. searchable snapshot 把数据主体放在对象存储（S3 等），frozen 节点本地只保留 partial cache。查询时按需从对象存储拉取相关 block 到本地缓存再检索，所以首次查询（冷启动）要走网络拉数据，慢到几秒甚至几十秒；命中缓存后变快。

6. cluster state 包含所有索引的 mapping 字段、index settings、alias、template、shard 分配等元数据。master 要把整个 cluster state 推送给所有节点；字段爆炸（如几万字段）会让 cluster state 涨到几百 MB，推送延迟、网络压力、master 内存都受影响，变更也会串行化阻塞。

7. `awareness` 让同一 shard 的主副本尽量分布到不同 zone（软约束，zone 全挂时仍可能把副本分到其他 zone）。`awareness.force` 是硬约束：某 zone 完全不可用时，shard 不会迁到其他 zone。必须用 force 的场景：防止某 zone 短暂故障时副本全挤到剩余 zone，等 zone 恢复后又触发大规模双向同步打爆集群，也避免分布不均。

8. 可能原因：(1) master 与 data 角色混用，data 大查询/写入占满 heap，GC 长导致 master 心跳超时；(2) master 节点 GC 抖动（heap 过大或 cluster state 过大）；(3) cluster state 过大（字段爆炸/shard 太多）导致推送阻塞；(4) 网络抖动/丢包；(5) master 节点配置过低跟不上集群规模。

9. CCS（跨集群搜索）适合：多机房集群联邦查询、hot 集群与历史 cold 集群分离、跨业务/跨地域聚合查询、单集群超过 ~500 节点时拆成多集群。额外延迟来自：协调节点要把请求转发到远程集群、跨集群（常跨地域）的网络 RTT、以及多集群结果的二次归并。

10. 拓扑：3 个 master-only（隔离选举）；一组 data_hot（NVMe，承担最近写入与近 7 天热查询，refresh 1s，replica 1）；一组 data_warm（SSD，7-30 天，shrink+force_merge，replica 1）；一组 data_cold（HDD，30-90 天，replica 0 + searchable snapshot 保护）；2 个 coordinating-only 应对查询前端；ILM：hot 按天 rollover、3d→warm、~7d 后查询变少迁更冷 tier、90d delete。容量按单 shard 30-50GB、每天 1TB ≈ 20+ 主 shard 规划。
