...
1. 向量检索用 embedding 模型把文本编码成稠密向量，语义相近的文本向量在空间中接近（cosine 距离小），所以"苹果手机"和"iPhone"、"失眠"和"睡眠障碍"即使无共同词也能匹配。BM25 是词袋统计模型，只看词项是否共现与 TF/IDF，不理解语义，没有共同词（或同义词表）就匹配不上。

2. HNSW 是多层图：高层节点稀疏、负责快速跨大距离的粗导航，逐层下降到底层（含全部节点、密集连接）做精确局部搜索，查询复杂度约 O(log N)。每个节点出现在前 k 层，层数按几何分布随机抽样（每升一层概率约 1/m 相关），越高层节点越少，从而形成稀疏-密集的层级结构。

3. int8 量化精度损失通常 < 5%；bbq（1-bit 二值量化）损失约 5-15%。bbq 必须配合 oversample rescoring：用量化向量多召回候选（如 oversample 3-5 倍），再用 float32 全精度对候选重排取最终 top k，否则召回率会明显下跌。

4. num_candidates 设太小：每 shard 探索的候选少，HNSW 可能找不到真正的近邻，召回率低（但快）。设太大：探索更多节点，召回率接近暴力搜索但查询变慢、CPU 上升。经验从 `max(100, k×10)` 起步，按业务 NDCG 满意度再调。

5. 不同 retriever（BM25 vs 向量）的分数尺度差异巨大，加权分数融合需要先归一化、调权重，且尺度不稳定。RRF 只用排名（rank=1,2,3...）计算 `Σ 1/(k+rank)`，天然消除尺度差异、无需归一化、对权重不敏感，是 hybrid search 的事实标准。

6. chunking 决定每个向量编码的语义单元粒度，直接影响召回质量和上下文完整性。太小（< ~100 字）：缺乏上下文，向量语义不准、检索到的片段信息不足；太大（> ~2000 字）：单 chunk 混杂多个主题，向量被"平均"得不集中，匹配精度下降。经验 300-800 字。

7. ELSER（稀疏向量）直接用 ES 倒排索引（term+权重），无需 HNSW，内存较少、与 BM25 天然可融合、可解释性好，但只能在 ES 内、跨语言一般。dense vector 用 HNSW 图，内存较多、查询快、需 RRF 才能和 BM25 融合、跨语言看 embedding 模型、部署灵活。英文为主不想调 HNSW 选 ELSER；多语言、大规模选 dense vector。

8. float32：1 亿 × 768 × 4B ≈ 307GB（约 300GB）。int8：1 亿 × 768 × 1B ≈ 77GB。bbq（1 bit）：1 亿 × 768/8B ≈ 9.6GB。再加 HNSW 图开销（约向量本身的 1.5 倍）后实际更高，但量化能把内存从数百 GB 降到几十 GB / 十几 GB。

9. 主要权衡：精度/召回 vs 内存与速度。1536 维表达能力更强、召回率更高，但每向量占用更多内存、HNSW 索引更大、查询更慢、写入更慢；384 维省内存、更快，但语义表达能力弱、召回略低。大规模（上亿）倾向小维度，质量优先且规模可控时用高维。

10. RAG 设计：
- **索引**：单索引（100 万 chunk 量级，shard 数 = 数据节点数即可），chunk 字段 `text`（text + ik_max_word，兼顾中文）、`vec`（dense_vector，dims 按 BGE-M3=1024，`int8_hnsw` 量化，cosine），`doc_id`/`chunk_idx`/`metadata`(flattened)/`@timestamp`。embedding 选 BGE-M3（多语言）。
- **检索 DSL**：用 retriever 做 hybrid——BM25（match text）+ kNN（query_vector_builder 调 BGE-M3，k=30, num_candidates=100），RRF 融合（rank_window_size 60）。
- **rerank**：外层 text_similarity_reranker 用 cross-encoder（如 bge-reranker）对 top 30-50 精排，返回 top 5 喂 LLM。
- **延迟**：100 万 chunk + int8 量化，HNSW 图可全驻 page cache，单 shard kNN 几毫秒；rerank top 30-50 约几十到一两百 ms，整体可控在 P95 < 500ms。准确度优先时 num_candidates 适当加大并保留 cross-encoder rerank。
