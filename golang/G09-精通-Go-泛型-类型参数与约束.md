...
设计决策：Go 团队认为方法上加类型参数会引入显著的类型检查复杂度，暂时不开放。

> 时效注记（2026-05）：Go 团队已于 2026-03 接受「为方法添加类型参数」提案（issue #77273，预计最早 Go 1.27 起支持，尚未最终承诺）。截至 Go 1.26 仍未落地；本节限制对稳定版仍然成立。注意：泛型接口方法（5.3）仍不在该提案范围内。
...
4. **不要给方法加自己的类型参数**：截至 Go 1.26 不支持（Go 团队已于 2026-03 接受相关提案，预计最早 Go 1.27 落地）；现阶段用顶层泛型函数代替。
