...
> 每章 5 道题，共 155 题。答案在最末尾"参考答案"章节。
...
## G31 — Socket 与 WebSocket

**31.1 ⭐⭐⭐** 服务端 `conn.Read(buf)` 返回 n=7，客户端刚好 `Write` 了一条 7 字节消息。能断定读到了完整一条消息吗？
- A. 能，n 就是消息长度
- B. 不能，TCP 是字节流，不保证消息边界
- C. 能，只要没开 Nagle
- D. 取决于 buf 大小

**31.2 ⭐⭐⭐⭐** WebSocket 协议要求客户端对 payload 做掩码（mask），目的是？
- A. 加密传输内容
- B. 校验数据完整性
- C. 防止中间代理被缓存投毒
- D. 压缩数据

**31.3 ⭐⭐⭐⭐⭐** 关于 WebSocket 的同源策略，正确的是？
- A. 浏览器会对 WebSocket 握手做 CORS 预检
- B. 不受同源策略保护，服务端必须自己校验 Origin，否则有 CSWSH 风险
- C. 同源策略自动阻止跨站 WebSocket 连接
- D. 只要用了 wss:// 就安全

**31.4 ⭐⭐⭐⭐** 两个 goroutine 同时向同一个 WebSocket 连接 `Write`，会发生什么？
- A. 库内部有锁，安全
- B. 帧交错产生非法流，对端断开（gorilla 直接 panic）
- C. 后一个写会阻塞等待
- D. 消息会自动合并

**31.5 ⭐⭐⭐** 服务端日志里大量 close code `1006`，说明？
- A. 客户端主动发了 1006 关闭帧
- B. 鉴权失败
- C. 消息过大被拒
- D. 本地库生成的保留码，代表没走关闭握手就异常断开
...
### G31
31.1 **B**（TCP 是字节流，n 只代表读到的字节数，可能粘包或半包）
31.2 **C**（防代理缓存投毒；key 明文在帧头，无加密价值）
31.3 **B**（WebSocket 握手不受 SOP 限制且自动带 Cookie，服务端必须校验 Origin）
31.4 **B**（一个连接只能一个 goroutine 写 → writePump 模式）
31.5 **D**（1006 是保留码，本地生成，永不出现在线上帧中）
...
| 分数 | 评价 |
|---|---|
| 140+/155（>90%） | 🏆 精通—— Go 高级工程师候选 |
| 109-139（70-89%） | 🥇 熟练—— 能胜任生产开发 |
| 78-108（50-69%） | 🥈 入门—— 重点补足薄弱章节 |
| <78（<50%） | 📖 建议重读对应章节 + 动手实践 |
